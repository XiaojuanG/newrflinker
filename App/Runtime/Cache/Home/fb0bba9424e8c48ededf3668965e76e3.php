...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/rflinker/Public/qiantai/css/commonDownload.css">
    ...
    <!--/ 导航-->
			...
			<div class="scrollable" id="scrollable">
				...
				<div id="carousel-example-generic" class="carousel slide banner" data-ride="carousel">
					...
					<div class="carousel-inner" role="listbox">
						<div class="item active">
							<img src="/rflinker/Public/qiantai/img/banner1.jpg" alt="">
				...
				<div class="news col-xs-12" style="background-color: #eee;padding-top:15px;">
					<div class="container">
						<!-- 按钮触发模态框 -->
						<a class="vip-btn-a" style="margin:0px 16px;" type="button" id="vipDownloadBtn" data-toggle="modal" data-target="#myModal"><?php echo (L("supportservicesdemo_downvip")); ?></a>
						<!-- 模态框（Modal） -->
						...
						<div class="row news-content" style="padding-top:25px;">
							...
							<div class="col-xs-12">
								...
								<?php if(is_array($supports)): $i = 0; $__LIST__ = $supports;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><a href="/rflinker/Public/Uploads/<?php echo ($val['file']); ?>" target="view_window"><li class="col-md-4 col-xs-12"><?php echo ($val['stname']); ?><i class="iconfont">&#xe615;</i></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
								</ul>
							</div>
    ...
    $('.matter-button').on('click', function() {
    	...
    	if(valid && requestData.matter == ''){ 
        	...
        	$('.vipbutton').on('click', function() {
			  ...
			  $codelink = "<?php echo U('Codeinfo/Index');?>";
	            ...
	            if(valid && requestData.vipname.length <= 0){
	                valid = false;
	                var tips = $(".vipname").attr("data-foolish-msg");
	                $(".vipname").focus();
	            }
	            var code = requestData.vipname;
				     $.ajax({
							type:"POST",
							url:"/rflinker/index.php/Home/Codeinput/code",
							dataType: "json",
							data:{"code":code},
							success: function(data){
                                 if(data == 1){
								     window.location.href= $codelink;
										  ...
										  layer.open({
											  content: '<?php echo (L("supportservicesdemo_isyzm")); ?>'
												,skin: 'msg'
												,time: 2 //2秒后自动关闭
											});
